...
export default {
    ...
    created(){
        ...
        this.favoriteMovies = JSON.parse(minhaLista)
    },
    methods:{
        deleteMovie(id){
            console.log(id)
            let filtroFilmes = this.favoriteMovies.filter((movie) => {
                return (movie.id !== id)
            })
            this.favoriteMovies = filtroFilmes
    ...
    watch: {
        favoriteMovies(){
            localStorage.setItem('myFilm', JSON.stringify(this.favoriteMovies))
        }
    }
}
</script>
