<template>
    <div id="watchList">
        <h3>Filmes que quero assistir</h3>
        <ul>
...
export default {
    ...
    created(){
        let myWatchList = JSON.parse(localStorage.getItem('toWatch'))
        this.watchList = myWatchList
    },
    methods: {
        deleteMovie(id){
            console.log('deletar movie id ', id)

            let filteredMovies = this.watchList.filter((movie) => {
                return (movie.id !== id)
            })

            this.watchList = filteredMovies
        },
    },
    ...
    watch: {
        watchList(){
            console.log('mudou')
            localStorage.setItem('toWatch', JSON.stringify(this.watchList))
        }
    }
